...
private[spark] class TaskSetManager(
  ...
  // Tripod
  def reorderTasksByCacheCondition(originArray: ArrayBuffer[Int]): ArrayBuffer[Int] = {
    var result = ArrayBuffer[Int]()
    // tasks uncached
    for(idx <- originArray) {
      if (tasks(idx).cached == 0) {
        result += idx
      }
    }
    // tasks cached
    for(idx <- originArray) {
      if (tasks(idx).cached == 1) {
        result += idx
      }
    }
    return result
  ...
  def sortByCachedCondition(cachedCondition: ArrayBuffer[Int]): Unit = {
    logInfo("Tripod: sort by cached condition for stage " + stageId.toString)
    logInfo(s"Tripod : cache condition is ${cachedCondition.toString()}")
    for (task <- tasks) {
      if (cachedCondition contains task.partitionId) {
        //        logInfo("Tripod: sort by cached condition for stage {}".format(stageId))
        task.cached = 1
      }
    }
    for (task <- tasks) {
      logWarning(s"Tripod : task ${task.partitionId} cache conditiion is ${task.cached}")
    }

    for (executor <- pendingTasksForExecutor.keys) {
      pendingTasksForExecutor(executor) = reorderTasksByCacheCondition(pendingTasksForExecutor(executor))
    }

    for (host <- pendingTasksForHost.keys) {
      pendingTasksForHost(host) = reorderTasksByCacheCondition(pendingTasksForHost(host))
    }

    for (rack <- pendingTasksForRack.keys) {
      pendingTasksForRack(rack) = reorderTasksByCacheCondition(pendingTasksForRack(rack))
    }

    //    pendingTasksWithNoPrefs.sortWith(tasks(_).cached < tasks(_).cached)
    pendingTasksWithNoPrefs = reorderTasksByCacheCondition(pendingTasksWithNoPrefs)

    allPendingTasks = reorderTasksByCacheCondition(allPendingTasks)

    logWarning("Tripod : Sorted results : ")
    logWarning("Tripod : tasks : " + tasks.toString)
    logWarning("Tripod : pendingTasksForExecutor : " + pendingTasksForExecutor.toString())
    logWarning("Tripod : pendingTasksForHost : " + pendingTasksForHost.toString())
    logWarning("Tripod : pendingTasksForRack : " + pendingTasksForRack.toString())
    logWarning("Tripod : pendingTasksWithNoPrefs : " + pendingTasksWithNoPrefs.toString())
    logWarning("Tripod : allPendingTasks : " + allPendingTasks.toString())
  }
