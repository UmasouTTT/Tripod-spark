    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        if (sc.stagePriority.nonEmpty) {
          ...
          logWarning("Tripod: Sort stages by id ...")
        ...
        if (missing.isEmpty) {
          ...
          for (parent <- missing) {
            ...
            stage_priority += parent.tripodPriority
            stage_priority += "  "
          }
          logWarning("Tripod stage order: " + stage_parents)
          logWarning("Tripod priority: " + stage_priority)
          for (parent <- missing) {
            submitStage(parent)
          }
          waitingStages += stage
