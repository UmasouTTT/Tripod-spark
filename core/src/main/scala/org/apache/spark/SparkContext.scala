...
class SparkContext(config: SparkConf) extends Logging {
  ...
  def stageHasInput(id: Int): Boolean = {
    stagesHasInput contains id
  }

  def getRDDToStageId(): Map[Int, Int] = {
    this.synchronized{
      var result: Map[Int, Int] = Map()
      try{
        val source = Source.fromFile("/proj/ccjs-PG0/tpch-spark/TripodConfig/" +
          "rddToStageId.txt", "UTF-8")
//        val source = Source.fromFile("/Users/umasou/workSpace/tmp/TripodConfig/" +
//          "rddToStageId.txt", "UTF-8")
        for (line <- source.getLines().toArray) {
          val tmp = line.split(":")
          result += (tmp(0).toInt -> tmp(1).toInt)
        }

        source.close()
      } catch {
        case ex: FileNotFoundException => {
          logError("Missing file rddToStageId")
        }
      }
  ...
  def getStageIdToTripodId(): Map[Int, Int] = {
    this.synchronized{
      var result: Map[Int, Int] = Map()
      try{
        val source = Source.fromFile("/proj/ccjs-PG0/tpch-spark/TripodConfig/" +
          "stageIdToTripodId.txt", "UTF-8")
//        val source = Source.fromFile("/Users/umasou/workSpace/tmp/TripodConfig/" +
//          "stageIdToTripodId.txt", "UTF-8")
        for (line <- source.getLines().toArray) {
          val tmp = line.split(":")
        ...
        case ex: FileNotFoundException => {
          logError("Missing file stageIdToTripodId")
        }
  ...
  def getStagePriority(): Map[Int, Int] = {
    this.synchronized{
      var result: Map[Int, Int] = Map()
      try{
        val source = Source.fromFile("/proj/ccjs-PG0/tpch-spark/TripodConfig/" +
          "stagePriority.txt", "UTF-8")
//        val source = Source.fromFile("/Users/umasou/workSpace/tmp/TripodConfig/" +
//          "stagePriority.txt", "UTF-8")
        ...
        case ex: FileNotFoundException => {
          logError("Missing file stagePriority")
        }
  ...
  def getStagesCachedCondition(): Map[Int, ArrayBuffer[Int]] = {
    this.synchronized{
      var result: Map[Int, ArrayBuffer[Int]] = Map()
      try{
        val source = Source.fromFile("/proj/ccjs-PG0/tpch-spark/TripodConfig/" +
          "stagesCachedCondition.txt", "UTF-8")
//        val source = Source.fromFile("/Users/umasou/workSpace/tmp/TripodConfig/" +
//          "stagesCachedCondition.txt", "UTF-8")

        val fileContent: Array[String] = source.getLines().toArray
        source.close()
        for (line <- fileContent) {
          if (line.nonEmpty) {
            val tmp = line.split(":")
            val stageId = tmp(0).toInt
            var cached_partitions = new ArrayBuffer[Int]()
            if (tmp.size > 1) {
              if (tmp(1).nonEmpty) {
                val str_cached_partitions = tmp(1).split(",")
                for (partitionId <- str_cached_partitions) {
                  cached_partitions += partitionId.toInt
                }

              }
            }
            result += (stageId -> cached_partitions)
          }
        ...
        case ex: FileNotFoundException => {
          logError("Missing file stagesCachedCondition")
        }
  ...
  def getStagesHasInput(): ArrayBuffer[Int] = {
    this.synchronized{
      var result = new ArrayBuffer[Int]()

      try{
        val source = Source.fromFile("/proj/ccjs-PG0/tpch-spark/TripodConfig/" +
          "stagesHasInput.txt", "UTF-8")
//        val source = Source.fromFile("/Users/umasou/workSpace/tmp/TripodConfig/" +
//          "stagesHasInput.txt", "UTF-8")
        ...
        case ex: FileNotFoundException => {
          logError("Missing file stagesHasInput")
        }
