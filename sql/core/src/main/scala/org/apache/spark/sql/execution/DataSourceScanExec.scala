...
case class FileSourceScanExec(
  ...
  private def createBucketedReadRDD(
    ...
    // Tripod
    logWarning("Tripod: createBucketedReadRDD ... ")
    val files_path = new StringBuffer("FilePaths:$")
    ...
    logWarning("Tripod rdd inf : ...")
    for (partition <- filePartitions) {
      for (file <- partition.files) {
        files_path.append(file.filePath + "-" + file.start + "-" + file.start + file.length + "$")
        logWarning(s"filePartition ${partition.index} 's " +
          s"files is : ${file.filePath}_${file.start}_" + (file.start + file.length).toString)
      }
    }

    new FileScanRDD(fsRelation.sparkSession, readFile, filePartitions).setName(files_path.toString)
  }
  ...
  private def createNonBucketedReadRDD(
    ...
    new FileScanRDD(fsRelation.sparkSession, readFile, partitions).setName(files_path.toString)
  }
